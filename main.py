...
class UserFedRL:
    ...
    def train_model(self, h_step=3, num_epochs=10, lr=1e-3, device="cpu"):
        ...
        try:
            ...
            print(f"Sample batch shapes - s0: {s0.shape}, a_seq: {a_seq.shape}, s_seq: {s_seq.shape}")
        ...
        for epoch in range(num_epochs):
            ...
            for s0, a_seq, s_seq in self.dataloader:
                ...
                for h in range(h_step):
                    ...
                    if h == 0 and epoch == 0 and batch_count == 1:
                        print(f"Step {h} - s_current: {s_current.shape}, a_t: {a_t.shape}")
                    
                    # 다음 state 예측
                    s_next, _ = self.model(s_current, a_t) #s_next랑 reward 둘다 output이니까. 
            ...
            print(f"[User Model Epoch {epoch+1}] Loss: {total_loss:.4f}, Batches: {batch_count}")
...
class Server:
    def __init__(self, actor, critic, model, datasetNames):
        ...
        for i, datasetName in enumerate(datasetNames):
            try:
                ...
                print(f"Creating models for user {i} - State dim: {state_dim}, Action dim: {action_dim}")
    ...
    def distribute_parameters(self):
        ...
        for user in self.users:
            # Actor 모델 파라미터 복사
            for user_param, server_param in zip(user.actor.parameters(), self.actor.parameters()):
                user_param.data.copy_(server_param.data)
            
            # Critic 모델 파라미터 복사
            for user_param, server_param in zip(user.critic.parameters(), self.critic.parameters()):
                user_param.data.copy_(server_param.data)
                
            # Model 파라미터 복사 (필요한 경우)
            for user_param, server_param in zip(user.model.parameters(), self.model.parameters()):
                user_param.data.copy_(server_param.data)
    ...
    def aggregate_parameters(self):
        ...
        for user_idx, user in enumerate(self.users):
            # Actor 파라미터 집계
            for server_param, user_param in zip(self.actor.parameters(), user.actor.parameters()):
                server_param.data.add_(user_param.data * weights[user_idx])
            
            # Critic 파라미터 집계
            for server_param, user_param in zip(self.critic.parameters(), user.critic.parameters()):
                server_param.data.add_(user_param.data * weights[user_idx])
...
class Model(nn.Module):
    ...
    def forward(self, state, action):
        try:
            ...
            print(f"state: {type(state)}, shape: {state.shape if hasattr(state, 'shape') else 'unknown'}")
            print(f"action: {type(action)}, shape: {action.shape if hasattr(action, 'shape') else 'unknown'}")
            raise
...
class SequenceTransitionDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, h_step):
        ...
        if hasattr(dataset[0], 'observations'):
            ...
            raise ValueError("Dataset format not recognized. Need either 'observations' or 'observation' attribute")
        ...
        print(f"Created {len(self.samples)} transition samples from {len(dataset)} episodes")
    ...
    def __getitem__(self, idx):
        ...
        # 데이터의 형태 확인 및 디버깅
        if idx == 0:
            print(f"First sample - state shape: {state_t.shape if hasattr(state_t, 'shape') else 'unknown'}")
...
def getDatasetInfo(datasetName):
    ...
    try:
        ...
        # 상태와 행동 공간 차원 추출
        if hasattr(dataset, 'observation_space') and hasattr(dataset.observation_space, 'shape'):
